...
{	
	...
	private void play_game(Bowler currentThrower)
	{
		canThrowAgain = true;
	...
	{
		Vector printVector;	
		EndGameReport egr = new EndGameReport( ((Bowler)party.getMembers().get(0)).getNickName() , party);
	...
	private void wait_untill_unhalted()
	{
		while (gameIsHalted) 
		{
			try 
	...
	private void frame_ten_task(PinsetterEvent pe)
	{
	...
	private LaneEvent lanePublish(  ) 
	{
		...
		details[0]=bowlIndex;
		details[1]=frameNumber+1;
		details[2]=ball;
		details[3]=gameIsHalted ? 1 : 0;
		LaneEvent laneEvent = new LaneEvent(party, details,currentThrower, cumulScores, scores);
		return laneEvent;
	}

	
	private void calculate_increase_score(int[][] cumulScores , int[] curScore , int i)
	{
	...
	private void add_normal(int i, int[] curScore)
	{
		if ( i/2 == 0 ) 
		{
	...
	private void add_final_frame(int i, int[] curScore)
	{
		if(i==18)
		{
	...
	private void calculate_increase_score_normally(int[][] cumulscore, int[] curScore, int i)
	{
		if( i%2 == 0 && i < 18)
		{
			add_normal(i,curScore);
		}
	...
	private int calc_srike_balls(int[] curScore, int strikeballs, int i)
	{
		if (curScore[i+2] != -1) 
		{
			strikeballs = 1;
	...
	private boolean check_firstball_strike(int i, int current, int[] curScore)
	{
		return i < current && i%2 == 0 && curScore[i] == 10  && i < 18;
	}
	
	private boolean check_secondball_notcurrent(int i, int current, int[] curScore)
	{
		return i%2 == 1 && curScore[i - 1] + curScore[i] == 10 && i < current - 1 && i < 19;
	}
	...
	private int getScore( Bowler Cur, int frame) 
	{
		...
		{
			//Spare:
			if( check_secondball_notcurrent(i,current,curScore))
			{
			...
			else if(check_firstball_strike(i,current,curScore))
			{
				strikeballs = 0;
				//This ball is the first ball, and was a strike.
				//If we can get 2 balls after it, good add them to cumul.
				strikeballs = calc_srike_balls(curScore,0,i);
				if (strikeballs == 2)
				{
					//Add up the strike.
					//Add the next two balls to the current cumulscore.
					calculate_increase_score(cumulScores,curScore,i); 
				} 
				else 
				{
					break;
			...
			{ 
				...
				calculate_increase_score_normally(cumulScores,curScore,i);
