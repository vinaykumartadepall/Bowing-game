...
public class LaneView implements LaneObserver, ActionListener {
	...
	private void wait_untill(Boolean initDone)
	{
		while (!initDone)
		{
	...
	private void update_with_characters(LaneEvent le, int k)
	{
		for (int i = 0; i < 21; i++)
		{
	...
	private void update_view(LaneEvent le, int k)
	{
	...
	public void receiveLaneEvent(LaneEvent le) {
		if (lane.isPartyAssigned())
		{
			...
			wait_untill(initDone);


			if (le.getFrameNum() == 1 && (le.getBall()*le.getBall() + le.getIndex()*le.getIndex() == 0))
			{
